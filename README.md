...
2. рассчитывает raw-заполненность;
3. применяет EMA с `alpha=0.3`;
4. вычисляет разницу температур;
5. вычисляет FireScore;
6. ведёт streak превышений;
7. после двух превышений создаёт `FIRE_RISK`;
8. создаёт `CLOSE_LID`;
9. отправляет команду подключённому устройству.

Формулы:

```text
raw_fill = (100 - distance) / (100 - 10) * 100
...
```text
ws://<компьютер>:8000/ws/device/municipal-rio-001
```
...
Backend сохраняет кадр в `static/vision` и создаёт `ILLEGAL_DUMP`, если demo-детектор обнаружил навал.
...
- `/api/vision/frame` использует mock-детекцию незаконного навала;
